...
interface SystemOverviewProps {
  buoys: BuoyData[];
  selectedZone: Zone;
  sensor?: SensorApiResponse; // or better: SensorApiResponse if you have the type
}
...
export function SystemOverview({ buoys, selectedZone, sensor }: SystemOverviewProps) {
  ...
  const cards = [
    ...
    ,
     {
      title: 'Avg Turbidity',
      value: sensor?.Turbidity_Readings?.[0]?.NTU_Value
        ? sensor.Turbidity_Readings[0].NTU_Value.toFixed(2)
        : '—',
